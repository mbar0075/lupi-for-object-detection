...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    ax[1].imshow(img[:, :, :3])  # Plot only the first three channels
    ax[1].axis("off")
...
def plot_images(
    ...
    # Ensure only the first three channels (RGB) are used
    if images.shape[1] > 3:
        images = images[:, :3, :, :]  # Use only the first 3 channels (RGB)
        # # Transpose to b, g, r order
        # images = images[:, [2, 1, 0], :, :]  # BGR
